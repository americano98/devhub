...
describe("static MVP directory", () => {
  it("contains the current official Databricks roster in source order", () => {
  ...
  it("uses official records when the backend snapshot differed", () => {
    const people = getMvpDirectory();
    const names = people.map(({ name }) => name);

    expect(names).not.toContain("Abishek Subramanian");
    expect(names).not.toContain("Szymon Dybczak");
    expect(
      people.find(({ name }) => name === "Dr. Alan L. Dennis")?.headline,
    ).toBe("Director I Author");
  });
  ...
  it("keeps local public images and profile destinations independent of the backend", () => {
    ...
    for (const person of people) {
      ...
      expect(
        existsSync(resolve(process.cwd(), "public", person.photoUrl.slice(1))),
        person.name,
      ).toBe(true);
      for (const url of [
